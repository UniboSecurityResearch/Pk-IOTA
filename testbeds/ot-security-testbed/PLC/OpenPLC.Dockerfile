...
RUN apt-get update \
 ...
 && cd OpenPLC_v3 \
 && sed -i 's/-DOPLC_ALL=ON /-DOPLC_ALL=OFF -DOPLC_ALL_SUPPORTING_TOOLS=ON -DOPLC_ALL_SUPPORTING_LIBRARIES=ON -DOPLC_BUILD_USER_APPLICATION=ON -DOPLC_BUILD_TESTS=OFF /' background_installer.sh \
 && sed -i 's@^\([[:space:]]*\)\./gg_unit_test$@\1if [ -x ./gg_unit_test ]; then ./gg_unit_test; else echo "Skipping gg_unit_test (not built)"; fi@' background_installer.sh \
