...
#RUN apt-get update && \
#    apt-get install --no-install-recommends --no-install-suggests -y \
#    build-essential libssl-dev zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev wget curl llvm libncurses5-dev libncursesw5-dev xz-utils tk-dev libffi-dev liblzma-dev python3-openssl git python2.7-dev libnetfilter-queue-dev && \
...
#    pyenv global 3.6.13 && \
#    pip3.6 install NetfilterQueue && \
#    pip3.6 install scapy
...
RUN apt-get update && \
    apt-get install --no-install-recommends --no-install-suggests -y \
      ...
      libnetfilter-queue-dev  \
      git && \
    python3 -m pip install --break-system-packages cython scapy && \
    (python3 -m pip install --break-system-packages NetfilterQueue || \
     echo "NetfilterQueue skipped: incompatible with current Python/Kali rolling base")
