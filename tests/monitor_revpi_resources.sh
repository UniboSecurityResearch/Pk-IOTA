...
function print_stats {
  ...
  voltage_mean=$(echo "${voltage_vals[@]}" | awk '{ sum=0; for (i=1; i<=NF; i++) sum+=$i; print sum/NF }')
  ...
  voltage_max=$(echo "${voltage_vals[@]}" | awk '{ max=$1; for (i=2; i<=NF; i++) if ($i>max) max=$i; print max }')
  ...
  echo "Statistics for Voltage: Mean=$voltage_mean V, Max=$voltage_max V"
...
echo "Timestamp, PID1 CPU (%), PID1 Memory (MB), PID2 CPU (%), PID2 Memory (MB), Voltage (V)" > $LOGFILE
...
while ps -p $PID1 > /dev/null && ps -p $PID2 > /dev/null; do
  ...
  # Log the data
  echo "$timestamp, $cpu1, $mem1_mb, $cpu2, $mem2_mb, $voltage" >> $LOGFILE
